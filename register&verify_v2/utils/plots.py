...
def save_one_box(img_size, xyxy, im, file, gain=1.02, pad=10, square=False, BGR=False, save=True):
...
def get_one_box(img_size, xyxy, im, gain=1.02, pad=10, square=False, BGR=False):
    xyxy = torch.tensor(xyxy).view(-1, 4)
    b = xyxy2xywh(xyxy)  # boxes
    if square:
        b[:, 2:] = b[:, 2:].max(1)[0].unsqueeze(1)  # attempt rectangle to square
